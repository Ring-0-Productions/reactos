    ...
    _In_ PATAPORT_DEVICE_EXTENSION DeviceExtension)
{
    ...
    /* DeviceExtension->MultiSectorTransfer = AtaMaximumSectorsPerDrq(IdentifyData); */
    ...
    if (IdentifyData->Capabilities.LbaSupported)
    {
        ...
        if (AtaHas48BitAddressFeature(IdentifyData))
        {
            ...
            if (AtaHasForcedAccessUnitCommands(IdentifyData))
    ...
    _In_ BOOLEAN IsAtaDevice)
{
    ...
    if (!IsAtaDevice)
    {
        ...
        InquiryData->AdditionalLength = FIELD_OFFSET(INQUIRYDATA, VendorSpecific) - 4;
    ...
    AtaSwapIdString(InquiryData->ProductId,
                    RTL_FIELD_SIZE(INQUIRYDATA, ProductId) / 2);
    ...
    AtaSwapIdString(InquiryData->ProductRevisionLevel,
                    RTL_FIELD_SIZE(INQUIRYDATA, ProductRevisionLevel) / 2);
    ...
    _In_ ATA_SCSI_ADDRESS AtaScsiAddress)
{
    ...
    static ULONG IdeDeviceNumber = 0;
    ...
    _In_ PATAPORT_DEVICE_EXTENSION OldDeviceExtension)
{
    ...
    for (DeviceClass = DEV_ATA; DeviceClass <= DEV_ATAPI; ++DeviceClass)
    {
         ...
         *   the status register of device 0 stuck permanently at a value of 0,
    ...
    _In_ PIRP Irp)
{
    ...
    for (DeviceNumber = 0; DeviceNumber < MaxDevices; ++DeviceNumber)
    {
        for (Lun = 0; Lun < MaxLun; ++Lun)
        {
            ...
            /* TargetId is encoded in bits 3:0 for internal use only (PC-98 support) */
            EnumDeviceExtension->DeviceHead = AtaScsiAddress.TargetId;
