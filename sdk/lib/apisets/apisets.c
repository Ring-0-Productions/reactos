...
// The unittest wants to link against ntdll,
// but the apisets library is used _inside_ ntdll so we have to make the symbol work in both cases.
// Therefore, we ask the callers of this library to define this symbol (and initialize it with RtlCompareUnicodeString)
extern LONG (NTAPI *
ApisetpRtlCompareUnicodeString)(
...
)
{
    ...
    if (!_wcsnicmp(Tmp.Buffer + (Tmp.Length - ExtensionSize) / sizeof(WCHAR), L".dll", ExtensionSize / sizeof(WCHAR)))
    ...
    while (LBnd <= UBnd)
    {
        ...
        LONG result = ApisetpRtlCompareUnicodeString(&Tmp, &g_Apisets[Index].Name, TRUE);
        if (result == 0)
        {
